...
#Область Редактор 
&НаКлиенте
Процедура ИнициализацияРедактораЗапросов()
	
	Инфо = Новый СистемнаяИнформация();
	
	РедакторЗапросов().init(Инфо.ВерсияПриложения);  
	РедакторЗапросов().setLanguageMode("bsl_query");
	РедакторЗапросов().setOption("autoResizeEditorLayout", Истина);
	РедакторЗапросов().setOption("renderQueryDelimiters", Истина);
	РедакторЗапросов().setOption("generateModificationEvent", Истина);
	РедакторЗапросов().minimap(Ложь);
	РедакторЗапросов().clearMetadata();
	
	
	СохраненныйЗапрос = ТекстЗапроса.ПолучитьТекст();
	Если СохраненныйЗапрос <> Неопределено Тогда 
		РедакторЗапросов().updateText(СохраненныйЗапрос);
	КонецЕсли;	
КонецПроцедуры  

&НаКлиенте
Процедура ИнициализацияРедактораАлгоритма()
	
	Инфо = Новый СистемнаяИнформация();
	
	РедакторАлгоритма().init(Инфо.ВерсияПриложения);  
	РедакторАлгоритма().setOption("autoResizeEditorLayout", Истина);
	РедакторАлгоритма().setOption("renderQueryDelimiters", Истина);
	РедакторАлгоритма().setOption("generateModificationEvent", Истина);
	РедакторАлгоритма().clearMetadata();
...
Процедура ПриПолученииКаталогаВременныхФайлов(ИмяКаталога, ДопПараметры) Экспорт
	...
	КаталогИсходныхФайловРедактора = ИмяКаталога + "bsl_console\";
	Оповещение = Новый ОписаниеОповещения("ПослеСозданияКаталога", ЭтотОбъект);
	НачатьСозданиеКаталога(Оповещение, КаталогИсходныхФайловРедактора);
...
Процедура ПослеСозданияКаталога(ИмяКаталога, ДопПараметры) Экспорт
	...
	ФайлНаДиске = Новый Файл(КаталогИсходныхФайловРедактора);
...
Процедура ПослеПроверкиСуществованияКаталога(Существует, ДопПараметры) Экспорт
	...
	Если Существует Тогда
		...
		ФайлНаДиске = Новый Файл(КаталогИсходныхФайловРедактора + ВерсияОбработки + ".ver");
		ФайлНаДиске.НачатьПроверкуСуществования(Оповещение);
	Иначе		
...
Процедура ПослеПроверкиСуществованияФайлаВерсии(Существует, ДопПараметры) Экспорт
	...
	ФайлНаДиске = Новый Файл(КаталогИсходныхФайловРедактора + "index.html");
	ФайлНаДиске.НачатьПроверкуСуществования(Оповещение);
...
Процедура ПослеПроверкиСуществованияИндексногоФайла(Существует, ДопПараметры) Экспорт
	...
	Если Существует Тогда		
		...
		ТочкаВхода = КаталогИсходныхФайловРедактора + "index.html";
...
Процедура ПослеКопированияИндексногоФайла(СкопированныйФайл, ДопПараметры) Экспорт
	...
	ИсходныеФайлыРедактораЗагружены = Истина;
...
&НаКлиенте
Функция ЗагрузитьОбщийМодульПоИмени(КаталогИсходногоКода, ИмяМодуля, Глобальный, Редактор)
	...
	Если ФайлНаДиске.Существует() Тогда
		...
		ЗагруженоФункций = Редактор.parseCommonModule(ОбщийМодуль.Имя, ОбщийМодуль.Текст, Глобальный);
	КонецЕсли;
...
&НаКлиенте
Функция ЗагрузитьСтруктуруОбщегоМодуляПоЗапросу(ЧастиЗапроса, Редактор)
	...
	Если ЗначениеЗаполнено(КаталогИсходногоКода) И ЧастиЗапроса.Количество() = КоличествоЧастейЗапроса Тогда
		...
		ЗагруженоФункций = ЗагрузитьОбщийМодульПоИмени(КаталогИсходногоКода, ИмяМодуля, Ложь, Редактор);
...
&НаКлиенте
Функция ЗагрузитьМодульМенеджераИлиОбъектаПоИмени(КаталогИсходногоКода, СтруктураВыгрузки, Редактор)
	...
	Если ФайлНаДиске.Существует() Тогда
		...
		ЗагруженоФункций = Редактор.parseMetadataModule(Модуль.Текст, АдресЗагрузки);
	КонецЕсли;
...
&НаКлиенте
Функция ЗагрузитьОбъектМетаданныхПоЗапросу(ПараметрыЗапроса, Редактор)
	...
	Если ЗначениеЗаполнено(ОписаниеМетаданных) Тогда
		...
		Если ТипЗнч(Редактор.updateMetadata(ОписаниеМетаданных, АдресОбновления)) = Тип("Булево") Тогда
			Возврат Истина;
		КонецЕсли;
...
&НаКлиенте
Функция ЗагрузитьСписокМетаданныхПоЗапросу(ПараметрыЗапроса, Редактор)
...
&НаКлиенте
Функция РедакторЗапросов()
	
	Возврат Элементы.ПолеРедактораЗапросов.Документ.defaultView;
	
КонецФункции 

&НаКлиенте
Функция РедакторАлгоритма()
	
	Возврат Элементы.ПолеРедактораАлгоритма.Документ.defaultView;
...
Функция ИндексныйФайл()
	...
	Возврат КаталогИсходныхФайловРедактора + Формат(ТекущаяУниверсальнаяДатаВМиллисекундах(), "ЧГ=0") + ".html";
...
Процедура ПоместитьРедактируемыйЗапрос()
	Перем КодКурсорНачалоСтроки, КодКурсорНачалоКолонки, КодКурсорКонецСтроки, КодКурсорКонецКолонки;
	
	Если РедактируемыйЗапрос >= 0 И ПолеРедактораЗапросаИнициализировано И ПолеРедактораАлгоритмаИнициализировано Тогда
		стрТекстЗапроса = ТекстЗапроса.ПолучитьТекст();
		стрТекстАлгоритм = ТекстАлгоритм.ПолучитьТекст();
		
		ГраницыВыделенияРедактораЗапроса = РедакторЗапросов().getSelection();   
		КурсорНачалоСтроки = ГраницыВыделенияРедактораЗапроса.selectionStartLineNumber;
		КурсорНачалоКолонки = ГраницыВыделенияРедактораЗапроса.selectionStartColumn;
		КурсорКонецСтроки = ГраницыВыделенияРедактораЗапроса.positionLineNumber;
		КурсорКонецКолонки = ГраницыВыделенияРедактораЗапроса.positionColumn;
		
		ГраницыВыделенияРедактораАлгоритма = РедакторАлгоритма().getSelection();  
		КодКурсорНачалоСтроки = ГраницыВыделенияРедактораАлгоритма.selectionStartLineNumber;
		КодКурсорНачалоКолонки = ГраницыВыделенияРедактораАлгоритма.selectionStartColumn;
		КодКурсорКонецСтроки = ГраницыВыделенияРедактораАлгоритма.positionLineNumber;
		КодКурсорКонецКолонки = ГраницыВыделенияРедактораАлгоритма.positionColumn;
...
Процедура УстановитьДоступностьРедактированияЗапроса(Доступность)
	Если ПолеРедактораЗапросаИнициализировано Тогда
		РедакторЗапросов().setReadOnly(НЕ Доступность);
...
Процедура ИзвлечьРедактируемыйЗапрос(ЗапросКроме = Неопределено, ВосстановитьПозийиюРедактирования = Истина)
	...
	Если ПолеРедактораЗапросаИнициализировано Тогда
		РедакторЗапросов().setContent(стДанныеЗапроса.Запрос);
	КонецЕсли;
...
Процедура ВосстановлениеПозицииРедактирования()
	...
	стДанныеЗапроса = Запрос_ПолучитьДанныеЗапроса(ТекущаяСтрока);
	Если ПолеРедактораЗапросаИнициализировано Тогда
		РедакторЗапросов().setSelection(стДанныеЗапроса.КурсорНачалоСтроки, стДанныеЗапроса.КурсорНачалоКолонки, стДанныеЗапроса.КурсорКонецСтроки, стДанныеЗапроса.КурсорКонецКолонки);
	КонецЕсли; 
	
	Если ПолеРедактораАлгоритмаИнициализировано Тогда
		РедакторАлгоритма().setSelection(стДанныеЗапроса.КодКурсорНачалоСтроки, стДанныеЗапроса.КодКурсорНачалоКолонки, стДанныеЗапроса.КодКурсорКонецСтроки, стДанныеЗапроса.КодКурсорКонецКолонки);
	КонецЕсли;
	
	Если Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаЗапрос Тогда
		ТекущийЭлемент = Элементы.ПолеРедактораЗапросов;   
	ИначеЕсли Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаАлгоритм Тогда
		ТекущийЭлемент = Элементы.ПолеРедактораАлгоритма;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВерсияОбработки = ОбъектОбработки.СведенияОВнешнейОбработке().Версия;
	...
	АдресМакетаИсходныхФайловРедактора = ПоместитьВоВременноеХранилище(ПолучитьМакет("src"), Новый УникальныйИдентификатор());
КонецПроцедуры
	...
	Если РезультатЗакрытия <> Неопределено Тогда
		...
		ИначеЕсли ДополнительныеПараметры.Поле = "ТипЗначения" Тогда
			...
			Если РезультатЗакрытия.Свойство("ТекстЗапроса", стрТекстЗапроса) Тогда
				...
				РедакторЗапросов().setSelectionByLength(чРазмерТекста + 1, чРазмерТекста + 1);
				РедакторЗапросов().selectedText(стрТекстЗапроса);
...
Процедура СтруктураЗаписиРезультатаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка) 
	Если ПолеРедактораАлгоритмаИнициализировано Тогда    
		РедакторАлгоритма().selectedText(СтруктураЗаписиРезультатаПолучитьТекстВставки(ВыбраннаяСтрока));
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ПолеРедактораЗапросовДокументСформирован(Элемент)
	Если ИсходныеФайлыРедактораЗагружены Тогда		
		ИнициализацияРедактораЗапросов();		
		ПолеРедактораЗапросаИнициализировано = Истина;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПолеРедактораЗапросовПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если Событие <> Неопределено Тогда
		...
		Если ИмяСобытия = "EVENT_CONTENT_CHANGED" Тогда 
			ТекстЗапроса.УстановитьТекст(РедакторЗапросов().getText());
			ПоместитьРедактируемыйЗапрос();  
		КонецЕсли;
		
		Если ИмяСобытия = "EVENT_QUERY_CONSTRUCT" Тогда 
			Команда_КонструкторЗапроса();
		КонецЕсли;   
		
		Если ИмяСобытия = "EVENT_GET_METADATA" Тогда 
			ОбработкаСобытияПолученияМетаданных(Событие.params, РедакторЗапросов());
		КонецЕсли;
...
&НаКлиенте
Процедура ПолеРедактораАлгоритмаДокументСформирован(Элемент)
	Если ИсходныеФайлыРедактораЗагружены Тогда		
		ИнициализацияРедактораАлгоритма();		
		ПолеРедактораАлгоритмаИнициализировано = Истина;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПолеРедактораАлгоритмаПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	Событие = ДанныеСобытия.Event.eventData1C;
	Если Событие <> Неопределено Тогда
		...
		Если ИмяСобытия = "EVENT_GET_METADATA" Тогда 
			ОбработкаСобытияПолученияМетаданных(Событие.params, РедакторАлгоритма());
		КонецЕсли;
...
&НаКлиенте
Функция ЗагрузитьСтруктуруМодуляМенеджераИлиОбъектаПоЗапросу(ЧастиЗапроса, Редактор)
	...
	Если ЗначениеЗаполнено(КаталогИсходногоКода) И ЧастиЗапроса.Количество() = КоличествоЧастейЗапроса Тогда
		...
		Если ЗначениеЗаполнено(СтруктураВыгрузки.ИмяКаталога) Тогда
			...
			ЗагруженоФункций = ЗагрузитьМодульМенеджераИлиОбъектаПоИмени(КаталогИсходногоКода, СтруктураВыгрузки, Редактор);
...
&НаКлиенте
Процедура ОбработкаСобытияПолученияМетаданных(Событие, Редактор)
	ПараметрыЗапросаМетаданных = НРег(Событие.metadata);
	ВызыватьПодсказку = (Событие.trigger = "suggestion");
	ДанныеОбновлены = Ложь;
	
	Если 0 < СтрНайти(ПараметрыЗапросаМетаданных, ".") Тогда
		
		ЧастиЗапроса = СтрРазделить(ПараметрыЗапросаМетаданных, ".");
		
		Если ЧастиЗапроса[0] = "module" Тогда
			
			Если ЧастиЗапроса.Количество() = 2 Тогда
				ДанныеОбновлены = ЗагрузитьСтруктуруОбщегоМодуляПоЗапросу(ЧастиЗапроса, Редактор);
			Иначе
				ДанныеОбновлены = ЗагрузитьСтруктуруМодуляМенеджераИлиОбъектаПоЗапросу(ЧастиЗапроса, Редактор);
			КонецЕсли;
			
		Иначе
			
			ДанныеОбновлены = ЗагрузитьОбъектМетаданныхПоЗапросу(ПараметрыЗапросаМетаданных, Редактор);
			
		КонецЕсли;
		
	Иначе
		
		ДанныеОбновлены = ЗагрузитьСписокМетаданныхПоЗапросу(ПараметрыЗапросаМетаданных, Редактор);
		
	КонецЕсли;
	
	Если Событие.trigger = "suggestion" И ДанныеОбновлены Тогда
		Редактор.triggerSuggestions();
	КонецЕсли;
...
Функция ПолучитьКонструкторЗапроса(ТекстЗапроса)
	...
	Исключение
		...
		Если ЗначениеЗаполнено(НомерСтроки) И ПолеРедактораЗапросаИнициализировано Тогда  
			РедакторЗапросов().setSelection(НомерСтроки, НомерКолонки, НомерСтроки, НомерКолонки);
		КонецЕсли;
...
КонецФункции
&НаКлиенте
Процедура Команда_КонструкторЗапроса(Команда=Неопределено)
...
Процедура Команда_КонструкторЗапроса_ОповещениеЗакрытияКонструктора(стрТекстЗапроса, ТекущийЗапрос) Экспорт
	...
	Если ПолеРедактораЗапросаИнициализировано Тогда
		РедакторЗапросов().updateText(ТекстЗапроса.ПолучитьТекст());
	КонецЕсли;
КонецПроцедуры
...
Процедура ВыполнитьЗапрос(фИспользоватьВыделение)
	Если НЕ ПолеРедактораЗапросаИнициализировано Тогда Возврат КонецЕсли;
	ГраницыВыделенияРедактораЗапросов = РедакторЗапросов().getSelection();
	
	НачалоСтроки = ГраницыВыделенияРедактораЗапросов.selectionStartLineNumber;
	НачалоКолонки = ГраницыВыделенияРедактораЗапросов.selectionStartColumn;
	КонецСтроки = ГраницыВыделенияРедактораЗапросов.positionLineNumber;
	КонецКолонки = ГраницыВыделенияРедактораЗапросов.positionColumn;
	...
	Иначе
		...
		стрТекстЗапроса = РедакторЗапросов().selectedText();
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(стРезультат.ОписаниеОшибки) Тогда
		...
		Если ЗначениеЗаполнено(стРезультат.Строка) Тогда
			Если фВесьТекст Тогда
				РедакторЗапросов().setSelection(стРезультат.Строка, стРезультат.Колонка, стРезультат.Строка, стРезультат.Колонка);
			Иначе
			КонецЕсли;
		КонецЕсли;
	Иначе
...
Процедура Команда_ПараметрыЗаполнитьИзЗапроса(Команда)
	...
	Если ЗначениеЗаполнено(стОшибка) Тогда
		...
		Если ЗначениеЗаполнено(стОшибка.Строка) Тогда
			РедакторЗапросов().setSelection(стОшибка.Строка, стОшибка.Колонка, стОшибка.Строка, стОшибка.Колонка);
		КонецЕсли;
...
Процедура Команда_ЗаполнитьИзXML(Команда)
	...
	КонецЕсли;  
	РедакторЗапросов().setContent(ТекстЗапроса.ПолучитьТекст());
...
Процедура ДобавитьПереносСтрокВТекст(ЭлементТекст)
	ГраницыВыделения = ЭлементТекст.getSelection();
	НачальнаяСтрока = ГраницыВыделения.selectionStartLineNumber;
	НачальнаяКолонка = ГраницыВыделения.selectionStartColumn;
	КонечнаяСтрока = ГраницыВыделения.positionLineNumber;
	КонечнаяКолонка = ГраницыВыделения.positionColumn;
	
	ОбрабатываемыйТекст = Новый ТекстовыйДокумент;
	ОбрабатываемыйТекст.УстановитьТекст(ЭлементТекст.getText());
	...
	ЭлементТекст.updateText(ОбрабатываемыйТекст.ПолучитьТекст());
	ЭлементТекст.setSelection(НачальнаяСтрока, НачальнаяКолонка, КонечнаяСтрока, КонечнаяКолонка);
КонецПроцедуры
...
Процедура Команда_ДобавитьПереносСтрокВТекст(Команда)
	...
	Если Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаЗапрос Тогда
		ДобавитьПереносСтрокВТекст(РедакторЗапросов());
	ИначеЕсли Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаАлгоритм Тогда
		ДобавитьПереносСтрокВТекст(РедакторАлгоритма());
	КонецЕсли;    
...
Процедура Команда_УбратьПереносСтрокИзТекста(Команда)
	Если Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаЗапрос Тогда
		УбратьПереносСтрокИзТекста(РедакторЗапросов());
	ИначеЕсли Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаАлгоритм Тогда
		УбратьПереносСтрокИзТекста(РедакторАлгоритма());
	КонецЕсли;
КонецПроцедуры
&НаКлиенте
Процедура ДобавитьКомментированиеСтрокВТекст(ЭлементТекст)
	...
	ОбрабатываемыйТекст = Новый ТекстовыйДокумент;
	ОбрабатываемыйТекст.УстановитьТекст(ЭлементТекст.selectedText());
	
	Для й = 1 По ОбрабатываемыйТекст.КоличествоСтрок() Цикл
		ОбрабатываемыйТекст.ЗаменитьСтроку(й, "//" + ОбрабатываемыйТекст.ПолучитьСтроку(й));
	КонецЦикла;
	
	ЭлементТекст.selectedText(ОбрабатываемыйТекст.ПолучитьТекст());
	ЭлементТекст.setSelection(НачальнаяСтрока, НачальнаяКолонка, КонечнаяСтрока, КонечнаяКолонка);
...
Процедура Команда_ДобавитьКомментированиеСтрокВТекст(Команда)
	Если Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаЗапрос Тогда
		РедакторЗапросов().addComment();
	ИначеЕсли Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаАлгоритм Тогда
		РедакторАлгоритма().addComment();
	КонецЕсли;
КонецПроцедуры
...
Процедура Команда_УбратьКомментированиеСтрокИзТекста(Команда)
	Если Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаЗапрос Тогда  
		РедакторЗапросов().removeComment();
	ИначеЕсли Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаАлгоритм Тогда
		РедакторАлгоритма().removeComment();
	КонецЕсли;
КонецПроцедуры
&НаКлиенте
Процедура Команда_СинтаксическийКонтрольЗапроса(Команда)
	...
	Если ТипЗнч(Результат) <> Тип("Строка") Тогда
		...
		Если ЗначениеЗаполнено(Результат.Строка) Тогда 
			РедакторЗапросов().setSelection(Результат.Строка, Результат.Колонка, Результат.Строка, Результат.Колонка);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура Команда_ФорматироватьТекстЗапроса(Команда)
	...
	РедакторЗапросов().setSelection(1, 1, ТекстЗапроса.КоличествоСтрок() + 1, 1);
	РедакторЗапросов().setContent(Результат);
	ПоместитьРедактируемыйЗапрос();
	Модифицированность = Истина;
КонецПроцедуры
...
Процедура Команда_ВыполнитьОбработку(Команда)
	ТекстАлгоритм.УстановитьТекст(РедакторАлгоритма().getText());
...
Процедура ОкончаниеВыбораПредопределенного(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	Если ЗначениеЗаполнено(РезультатЗакрытия) Тогда
		...
		РедакторЗапросов().selectedText(РезультатЗакрытия.Результат);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Команда_ВставитьПредопределенноеЗначение(Команда)
	ГраницыВыделения = РедакторЗапросов().getSelection();
	НачальнаяСтрока = ГраницыВыделения.selectionStartLineNumber;
	НачальнаяКолонка = ГраницыВыделения.selectionStartColumn;
	КонечнаяСтрока = ГраницыВыделения.positionLineNumber;
	КонечнаяКолонка = ГраницыВыделения.positionColumn;
...
Процедура ОткрытьФормуСписка(Команда)
	ОткрытьФорму(ПолноеИмяФормы("ФормаВыборкиДанных"));
КонецПроцедуры
