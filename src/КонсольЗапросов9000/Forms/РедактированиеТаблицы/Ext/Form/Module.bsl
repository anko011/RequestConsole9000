...
&НаСервере
Функция ВыгрузкаЗаголовков();
	ТЗ = ТаблицаЗначений.Выгрузить(,);	
	
	МассивЗаголовков = Новый Массив;
	КоличествоКолонок =  ТЗ.Колонки.Количество() - 1; 
	
	Для Колонка = 0 По КоличествоКолонок Цикл
		МассивЗаголовков.Добавить(ТЗ.Колонки[Колонка].Имя);		
	КонецЦикла;
...
&НаКлиенте
Процедура КомандаТаблДокум(Команда);
	СписокЗаголовков = Новый СписокЗначений;
	СписокЗаголовков.ЗагрузитьЗначения(ВыгрузкаЗаголовков());
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Заголовки", СписокЗаголовков);
	
	ОткрытьФорму
	("ВнешняяОбработка.КонсольЗапросов9000.Форма.РедактированиеТабличногоДокумента",
	ПараметрыФормы,
	ЭтаФорма,,,,,
...
Процедура ОбработатьПодбор(АдресВХ) экспорт
	ТаблицаЗначОбработанная = ПолучитьИзВременногоХранилища(АдресВХ);
	
	Для Каждого ЭлементМассива Из ТаблицаЗначОбработанная Цикл
